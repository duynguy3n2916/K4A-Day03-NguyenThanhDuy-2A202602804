...
* [x] Khai báo Tool Schema cho `health_profile_query` và `schedule_training_session` trong `src/tools.py`.
...
* [x] Cấu hình chuẩn bị API Key trong file `.env`.
...
* [x] Kiểm tra tên Repo cá nhân đúng chuẩn: **`K4A-Day03-NguyenThanhDuy-2A202602804`**.
* [x] Chạy lệnh Git để commit mã nguồn hoàn thiện trên GitHub cá nhân:
  ```bash
  git add .
